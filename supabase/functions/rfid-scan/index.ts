...
serve(async (req) => {
  ...
  // Find student by RFID code in students table, then profiles table
  let { data: student } = await supabase.from("students").select("*").eq("rfid_code", rfid_code).single();
  if (!student) {
    const { data: profileStudent } = await supabase.from("profiles").select("*").eq("rfid_code", rfid_code).single();
    if (profileStudent) {
      student = {
        id: profileStudent.student_id || profileStudent.id,
        name: profileStudent.full_name,
  ...
  // Find session and get class_id
  const { data: session } = await supabase.from("attendance_sessions")
    .select("*").eq("session_code", session_code).eq("is_active", true).single();
  if (!session) {
    return new Response(JSON.stringify({ error: "Active session not found" }), { status: 404 });
  }
  ...
  const { error: attendanceError } = await supabase.from("attendance_records").insert(record);
  if (attendanceError) {
  ...
  return new Response(JSON.stringify({
    success: true,
    student: student ? { name: student.name, matric_number: student.matric_number, department: student.department }
      : { name: "Unknown Student", matric_number: "N/A", department: "N/A" },
    session_code: session_code
  }), { status: 200 });
});
